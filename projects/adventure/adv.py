...
class Stack():
    ...
    def push(self, value):
        ...
        self.stillPath.append(value.id)
...
def traverseMap(player):
    global graph
    global world

    stack = Stack(player.currentRoom)
    dirPriority = ['w', 's', 'n', 'e']
    inverse = {
        'n': 's',
        'w': 'e',
    ...
    while stack.size() > 0:
        ...
        #print('0 graph at step start:', graph)
        print('1 prevRoom:', prevRoom, "CURRENT ROOM:",
              room.id, "direction", direction)
        #print(f'1.1 RevPath: {stack.revPath} \n Path: {stack.totalPath}')
        rooms = room.getExits()
        if room.id not in graph:

            print(f"1.5 Exits: {rooms}")
            graph[room.id] = {}
            for ex in rooms:
                graph[room.id][ex] = '?'

            if type(prevRoom) == int:
                print(
                    f"2 the room is {room.id}: {graph[room.id]} and {inverse[direction]} is  room {prevRoom}")
                # setting the room to last location
                graph[room.id][inverse[direction]] = prevRoom
                print("3 prevRoom:", graph[room.id][inverse[direction]])
            print(f"3.5 Reverse Path is: {stack.path}")

            direction = 'w'
            print(
                f"3.6 Direction of the previous room: {stack.path[prevRoom]} direction: {direction}")
            if stack.path[prevRoom] != direction:
            ...
            if player.currentRoom.id == room.id:
                ...
                while player.currentRoom.id == room.id:
                    ...
                    if index > 3:
                        revPath = list(reversed(stack.revPath))
                        """ for room in revPath:
                            print('room', room)
                            for direction in dirPriority:
                                if direction in graph[room]:
                                    if graph[room][direction] == '?':
                                        print(f"unknown path at room {room} the return path is: {stack.revPath}")
                                        return """
                        ...
                        while unknownFound == False:
                            #print('3.601 LOOKING FOR UNKNOWN$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
                            for rm in graph:
                                
                                for i in graph[rm]:
                                    
                                    if graph[rm][i] == '?':
                                        #print(f"3.602 UNKNOWN ROOM IS {rm}: {graph[rm]}")
                                        somethingFound = True
                            ...
                            if 145 in graph:
                                print(f"145: {graph[145]}")
                            print(f"graph length: {len(graph)} steps: {len(stack.totalPath)}, revPathLength: {len(revPath)}index {idx}")
                            if idx > len(revPath) - 1:
                                q = Queue()
                                q.enqueue(player.currentRoom.id)
                                while q.size() > 0:
                                    r = q.dequeue()
                                    for dtn in r.getExits():
                                        if graph[r.id][dtn] == '?':
                                            direction = dtn
                                            unknownFound = True
                                            needsReset = True
                                            break
                                    for nbr in graph[r.id]:
                                        r.enqueue(nbr)
                            ...
                            for directn in dirPriority:

                                if directn in graph[curRoom]:

                                    if graph[curRoom][directn] == '?':
                                        # player.travel(directn)
                                        direction = directn
                                        unknownFound = True
                                        needsReset = True
                                        break
                        ...
                        print(
                            f"END BACKTRACK. Player location now: {player.currentRoom.id}")
                        break

                    print(
                        f"4 Trying to move {dirPriority[index]} from {room.id}")
                    if stack.path[prevRoom] == dirPriority[index] and stack.path[prevRoom] != 'e':
                        print(
                            f"4.1 Previous room: {stack.path[prevRoom]} Prev room in stack:{ graph[player.currentRoom.id][stack.path[prevRoom] ]}")
                        index += 1
                    if stack.path[prevRoom] == dirPriority[index] and stack.path[prevRoom] == 'e':
                        print(
                            f"4.2 Previous room: {stack.path[prevRoom]} Prev room in stack:{ graph[player.currentRoom.id][stack.path[prevRoom] ]}")
                        index += 1

                    print(
                        f"5 Path of Rooms is {stack.path}")
                    if stack.path[prevRoom] != 'e':
            ...
            if needsReset == True:
                ...
                currentRoom = player.currentRoom.id
                print(
                    f'CURRENT ROOM IS IN GRAPH {currentRoom in graph}*** \n *** \n ***')

        print(f"6.5 Exits for room {room.id} are now: {graph[room.id]}")
        hasUnvisited = False
        for ex in rooms:
            if graph[room.id][ex] == '?':
                hasUnvisited = True
        ...
        if len(stack.path) != 1:
            firstLoop = False
        print(f"{(hasUnvisited == True)} and {(firstLoop == False)} and {(currentRoom in graph)}")
        if hasUnvisited == True and firstLoop == False and (currentRoom in graph):
            print(f"7 NEXT LOOP ================ \n \n")
            ...
            direction = 'w'  # 1 ORIGINAL CONFLICTING LOCATION : 233
            firstCondition = False
            for direct in dirPriority:
                if direct in graph[room.id]:
                    if graph[room.id][direct] == '?':
                        print(
                            f"8.1 player is at Room {room.id}, currentID is {player.currentRoom.id}, direction ")
                        prevRoom = player.currentRoom.id
                        player.travel(direct)
                        stack.addToPath(prevRoom, inverse[direct])
                        print(
                            f"8.2 player is at Room {room.id}, currentID is {player.currentRoom.id}, direction ")
                        direction = direct
                        firstCondition = True
                        break
            if firstCondition != True:
                for direct in dirPriority:  # 2 NEXT CONFLICTING LOCATION : 240
                    if direct in graph[room.id]:

                        if stack.path[prevRoom] != direction and graph[room.id][direct] == '?':
                            print(
                                f"8.21 Prev room direction is {stack.path[prevRoom]} != current direction {direction}")
                            print('8.3 Conflicting case')
                            prevRoom = player.currentRoom.id
                            player.travel(direct)
                            stack.addToPath(prevRoom, inverse[direct])
                            direction = direct
            ...
            if player.currentRoom.id == room.id:
                ...
                while player.currentRoom.id == room.id:
                    ...
                    player.travel(dirPriority[index])
                    direction = dirPriority[index]
            print(f"10.1 the graph before setting:", graph[room.id])
            graph[room.id][direction] = player.currentRoom.id
            prevRoom = room.id
            print(
            ...
            stack.push(player.currentRoom)
            stack.addToPath(room.id, inverse[direction])
        if hasUnvisited == False and (currentRoom in graph):
            index = 0
            hibber = 0
            while player.currentRoom.id == room.id:
                    index += 1
                    ...
                    if index > 3:
                        ...
                        somethingFound = False
                        while unknownFound == False:
                            print('11.601 LOOKING FOR UNKNOWN$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
                            for rm in graph:
                            ...
                            curRoom = player.currentRoom.id
                            print(
                                f'11.61 now at room {curRoom}, {graph[curRoom]}')
                            for directn in dirPriority:
                                ...
                                if directn in graph[curRoom]:
                                    ...
                                    if graph[curRoom][directn] == '?':
                                        print(
                                            f" 11.62 unknown path at room {curRoom}; ")
                                        # player.travel(directn)
                                        direction = directn
                                        unknownFound = True
                                        needsReset = True
                        ...
                        print(
                            f"12 END BACKTRACK. Player location now: {player.currentRoom.id}")
                        break

                    print(
                        f"13 Trying to move {dirPriority[index]} from {room.id}")
                    if stack.path[prevRoom] == dirPriority[index] and stack.path[prevRoom] != 'e':
                        print(
                            f"13.1 Previous room: {stack.path[prevRoom]} Prev room in stack:{ graph[player.currentRoom.id][stack.path[prevRoom] ]}")
                        index += 1
                    if stack.path[prevRoom] == dirPriority[index] and stack.path[prevRoom] == 'e':
                        print(
                            f"13.2 Previous room: {stack.path[prevRoom]} Prev room in stack:{ graph[player.currentRoom.id][stack.path[prevRoom] ]}")
                        index += 1

                    print(
                        f"14 Path of Rooms is {stack.path}")
                    if stack.path[prevRoom] != 'e':
                        player.travel(dirPriority[index])
                        direction = dirPriority[index]
            prevRoom = player.currentRoom.id
...
#
# Choose the first vacant exit and go to it
#   #scale the graph going along the 'west wall' with direction priorities
#   #always go west
#   #direction priorities: west, south, north, left

# Update the old room stats to the graph
#   #add the current room to the last traversal direction in the prev room
# Update the graph with the new room stats
...
""" 
move to room, update
check if west is a mystery, etc through priorities
if no mysteries, do bfs
keep track of the path of along the way to the nearest neighbor 
 """

#   If you get to a room with no unknown exits
#   #search for the nearest room w/ un
# each room has a list of  exits to let you know where its going
# look in a room and explore if there are unexplored rooms; if so, move in a direction
