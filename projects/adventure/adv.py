            ...
            if player.currentRoom.id == room.id:
                ...
                while player.currentRoom.id == room.id:
                    ...
                    if index > 3:
                        ...
                        print(
                            f'index: {idx} location: {player.currentRoom.id}')
                            ...
                            for rm in graph:
                                ...
                                for i in graph[rm]:
                                    ...
                                    if graph[rm][i] == '?':
                                        somethingFound = True
                                        print(
                                            f"3.6001 unknown found at room {rm}")
                            if somethingFound == False:
                                # print(graph)
                                print(
                                    f'DONE: position{player.currentRoom.id} revPath: {revPath}')
                                #print(f"Final Path: {stack.totalPath}")

                                return stack.totalPath
                            # 3 NEXT CONFLICT LINE 148
                            print(
                                f"previous room is {prevRoom}, reverse path is {revPath}, length of revPath is {len(revPath)} current room is {player.currentRoom.id}: {graph[player.currentRoom.id]}")
                            prevRoom = player.currentRoom.id
                            player.travel(revPath[idx])
                            stack.addToPath(prevRoom, inverse[revPath[idx]])
                            stack.addToTotalPath(
                                revPath[idx])  # adding to path
                            curRoom = player.currentRoom.id
                            print(
                                f'3.601 Length of graph is {len(graph)}, original is {len(roomGraph)}, steps are {len(stack.totalPath)}')
                            print(f"Graph: {graph}")
                            world.printRooms()
                            #print(f'3.61 now at room {curRoom}, {graph[curRoom]}')
                            ...
                            for directn in dirPriority:
                                ...
                                if directn in graph[curRoom]:
                                    ...
                                    if graph[curRoom][directn] == '?':
                                        print(
                                            f" 3.62 UNKNOWN PATH at room {curRoom}; ")
                    ...
                    if stack.path[prevRoom] == dirPriority[index]:
                        print(
                            f"4.1 {stack.path[prevRoom]} == {dirPriority[index]}")
                        if index <= 3:
                            index += 1

                    print(
                        f"5 Path of Rooms is {stack.path} \n Travel Index is {index}")
                    if index <= 3:
                        prevRoom = player.currentRoom.id
                        player.travel(dirPriority[index])
                        stack.addToPath(prevRoom, inverse[dirPriority[index]])
