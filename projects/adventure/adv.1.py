...
for ext in player.currentRoom.getExits():
    unvisited.add(f'{player.currentRoom.id}{ext}')
...
while unvisited:
    rooms = [graph[player.currentRoom.id][i] for i in graph[player.currentRoom.id]]
    
    if '?' in rooms: 
        direction = 'w'
        currentRoom = player.currentRoom.id

        if 'n' in graph[currentRoom] and graph[currentRoom]['n'] == '?':
            direction = 'n'
        elif 'e' in graph[currentRoom] and graph[currentRoom]['e'] == '?':
            direction = 'e'
        elif 's' in graph[currentRoom] and graph[currentRoom]['s'] == '?':
            direction = 's'
        elif 'w' in graph[currentRoom] and graph[currentRoom]['w'] == '?':
            direction = 'w'
        
        unvisited.remove(f'{player.currentRoom.id}{direction}')
        ...
        if nextRoom not in graph:
            graph[nextRoom] = {
                x: '?' for x in player.currentRoom.getExits()
                }

        
        graph[currentRoom][direction] = nextRoom
        graph[nextRoom][inverse[direction]] = currentRoom
        
        for ext, value in graph[nextRoom].items():
            print(graph[nextRoom].items())
            if value == '?':
                unvisited.add(f'{nextRoom}{ext}')

        
        if f'{nextRoom}{inverse[direction]}' in unvisited:
            unvisited.remove(f'{nextRoom}{inverse[direction]}')

    
    else:
        currentRoom = player.currentRoom.id
        queue = Queue()
        for direction, room in graph[currentRoom].items():
            queue.put([[direction, room]])
        ...
        while not queue.empty():
            ...
            if '?' in [room for direction, room in graph[current[1]].items()]:
                ...
                for direction, room in graph[current[1]].items():
                    if room != currentRoom and room not in [y for x, y in current_path]:
